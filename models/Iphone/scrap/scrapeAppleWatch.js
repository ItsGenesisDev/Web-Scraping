...
const baseUrl = 'https://everymac.com/systems/apple/apple-watch/index-apple-watch-specs.html';

export async function scrapeAppleWatch() {
  try {
    ...
    const filePath = path.join(jsonsFolderPath, 'allSmartwatchInfo.json');
...
async function getDeviceInfo(url) {
  try {
    ...
    const modelDevice = $('#content11-title tbody tr td').eq(1).text().trim();
    const orderDevice = $('#content10-title tbody tr td').eq(1).text().trim();
    const dateDevice = $('#content1-title tbody tr td').eq(1).text().trim();
    const sizeDevice = $('#content5-title tbody tr td').eq(3).text().trim();
    const colorDevice = $('#content6-title tbody tr td').eq(1).text().trim();
...
module.exports = { scrapeAppleWatch, getDeviceInfo };
